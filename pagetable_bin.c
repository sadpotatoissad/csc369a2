...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		//when evict a page, set zero the valid bit of the PTE
		(coremap[frame].pte)-> frame & ~PG_VALID; 
		//if dirty, Write victim page to swap
		if ((coremap[frame].pte)-> frame & PG_DIRTY) {
			off_t swap_off; // ??how to know swap_off??
			swap_off = wap_pageout(frame, swap_offset);
			//stores the location of the page in the swap space in the PTE
...
char *find_physpage(addr_t vaddr, char type) {
	...
	if (!(pgdir[idx].pde & PG_VALID)){
		pgdir[idx].pde = init_second_level() //?? not sure about this		
	}
	...
	// else, page is not in memory. it's a miss
		...
		miss_count ++;
		//find a free frame_no or evict to get one for p
